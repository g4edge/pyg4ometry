    ...
    def mesh(self):
        ...
        for j0 in range(nslice):
            ...
            for i0 in range(nstack):
                ...
                xRMaxP1T1 = (pRtor + (pRmax * _np.cos(dTheta * i1))) * _np.cos(
                    pSPhi + dPhi * j1
                )
                yRMaxP1T1 = (pRtor + (pRmax * _np.cos(dTheta * i1))) * _np.sin(
                    pSPhi + dPhi * j1
                )
                zRMaxP1T1 = pRmax * _np.sin(dTheta * i1)

                xRMaxP1T2 = (pRtor + (pRmax * _np.cos(dTheta * i2))) * _np.cos(
                    pSPhi + dPhi * j1
                )
                yRMaxP1T2 = (pRtor + (pRmax * _np.cos(dTheta * i2))) * _np.sin(
                    pSPhi + dPhi * j1
                )
                zRMaxP1T2 = pRmax * _np.sin(dTheta * i2)

                xRMaxP2T2 = (pRtor + (pRmax * _np.cos(dTheta * i2))) * _np.cos(
                    pSPhi + dPhi * j2
                )
                yRMaxP2T2 = (pRtor + (pRmax * _np.cos(dTheta * i2))) * _np.sin(
                    pSPhi + dPhi * j2
                )
                zRMaxP2T2 = pRmax * _np.sin(dTheta * i2)

                xRMaxP2T1 = (pRtor + (pRmax * _np.cos(dTheta * i1))) * _np.cos(
                    pSPhi + dPhi * j2
                )
                yRMaxP2T1 = (pRtor + (pRmax * _np.cos(dTheta * i1))) * _np.sin(
                    pSPhi + dPhi * j2
                )
                ...
                xRMinP1T1 = (pRtor + (pRmin * _np.cos(dTheta * i1))) * _np.cos(
                    pSPhi + dPhi * j1
                )
                yRMinP1T1 = (pRtor + (pRmin * _np.cos(dTheta * i1))) * _np.sin(
                    pSPhi + dPhi * j1
                )
                zRMinP1T1 = pRmin * _np.sin(dTheta * i1)

                xRMinP1T2 = (pRtor + (pRmin * _np.cos(dTheta * i2))) * _np.cos(
                    pSPhi + dPhi * j1
                )
                yRMinP1T2 = (pRtor + (pRmin * _np.cos(dTheta * i2))) * _np.sin(
                    pSPhi + dPhi * j1
                )
                zRMinP1T2 = pRmin * _np.sin(dTheta * i2)

                xRMinP2T2 = (pRtor + (pRmin * _np.cos(dTheta * i2))) * _np.cos(
                    pSPhi + dPhi * j2
                )
                yRMinP2T2 = (pRtor + (pRmin * _np.cos(dTheta * i2))) * _np.sin(
                    pSPhi + dPhi * j2
                )
                zRMinP2T2 = pRmin * _np.sin(dTheta * i2)

                xRMinP2T1 = (pRtor + (pRmin * _np.cos(dTheta * i1))) * _np.cos(
                    pSPhi + dPhi * j2
                )
                yRMinP2T1 = (pRtor + (pRmin * _np.cos(dTheta * i1))) * _np.sin(
                    pSPhi + dPhi * j2
                )
