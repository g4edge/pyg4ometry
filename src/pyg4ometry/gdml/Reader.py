...
class Reader:
    ...
    def load(self):
        ...
        entities = {}
        en_block = _re.search(r"<!DOCTYPE(\s+)gdml([\s\S]*)>", start_block)
        ...
        for material in materials:
            ...
            if not material["components"]:
                ...
                for comp in material["components"]:
                    ...
                    if comp_type == "fraction":
                        ...
                        raise ValueError(
                            f"Unrecognised material component type: {comp_type}"
                        )
            ...
            for pname, pref in properties.items():
                if pref not in self._registry.defineDict or not isinstance(
                    ...
                    raise ValueError(
                        f"Referenced matrix {pref} not defined for property {pname} on material {name}"
                    )
    ...
    def parsePolyhedra(self, node):
        ...
        nside = _defines.Expression(
            f"{solid_name}_numSide", node.attributes["numsides"].value, self._registry
        )
    ...
    def parseGenericTrap(self, node):
        ...
        for i in range(1, 9):
            vx = _defines.Expression(
                f"{solid_name}_v{i}x", node.attributes[f"v{i}x"].value, self._registry
            )
            vy = _defines.Expression(
                f"{solid_name}_v{i}y", node.attributes[f"v{i}y"].value, self._registry
            )
            args.extend([vx, vy])
    ...
    def parseOpticalSurface(self, node):
        ...
        for pname, pref in properties.items():
            if pref not in self._registry.defineDict:
                raise ValueError(
                    f"Referenced matrix {pref} not defined for property {pname} on optical surface {solid_name}"
                )
