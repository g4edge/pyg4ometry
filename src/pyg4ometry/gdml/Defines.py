...
def upgradeToTransformation(var, reg, addRegistry=False):
    ...
    if isinstance(var[0], VectorBase):
        ...
        raise TypeError(f"Unknown rotation type: {type(var[0])}")
    ...
    if isinstance(var[1], VectorBase):
        ...
        raise TypeError(f"Unknown position type: {type(var[1])}")
...
class ScalarBase:
    ...
    def __neg__(self):
        ...
        v = Constant(
            f"var_neg_{v1}", f"-({v1})", registry=self.registry, addRegistry=False
        )
        return v
...
def sin(arg):
    ...
    v = Constant(f"sin_{v1}", f"sin({v1})", registry=arg.registry, addRegistry=False)
    return v
...
def cos(arg):
    ...
    """
    v1 = upgradeToStringExpression(arg.registry, arg)
    v = Constant(f"cos_{v1}", f"cos({v1})", registry=arg.registry, addRegistry=False)
    return v
...
def tan(arg):
    ...
    """
    v1 = upgradeToStringExpression(arg.registry, arg)
    v = Constant(f"tan_{v1}", f"tan({v1})", registry=arg.registry, addRegistry=False)
    return v
...
def asin(arg):
    ...
    """
    v1 = upgradeToStringExpression(arg.registry, arg)
    v = Constant(f"sin_{v1}", f"asin({v1})", registry=arg.registry, addRegistry=False)
    return v
...
def acos(arg):
    ...
    """
    v1 = upgradeToStringExpression(arg.registry, arg)
    v = Constant(f"cos_{v1}", f"acos({v1})", registry=arg.registry, addRegistry=False)
    return v
...
def atan(arg):
    ...
    """
    v1 = upgradeToStringExpression(arg.registry, arg)
    v = Constant(f"tan_{v1}", f"atan({v1})", registry=arg.registry, addRegistry=False)
    return v
...
def exp(arg):
    ...
    """
    v1 = upgradeToStringExpression(arg.registry, arg)
    v = Constant(f"exp_{v1}", f"exp({v1})", registry=arg.registry, addRegistry=False)
    return v
...
def log(arg):
    ...
    """
    v1 = upgradeToStringExpression(arg.registry, arg)
    v = Constant(f"log_{v1}", f"log({v1})", registry=arg.registry, addRegistry=False)
    return v
...
def log10(arg):
    ...
    v1 = upgradeToStringExpression(arg.registry, arg)
    v = Constant(
        f"log10_{v1}", f"log10({v1})", registry=arg.registry, addRegistry=False
    )
    return v
...
def sqrt(arg):
    ...
    """
    v1 = upgradeToStringExpression(arg.registry, arg)
    v = Constant(f"sqrt_{v1}", f"sqrt({v1})", registry=arg.registry, addRegistry=False)
    return v
...
def abs(arg):
    ...
    """
    v1 = upgradeToStringExpression(arg.registry, arg)
    v = Constant(f"abs_{v1}", f"abs({v1})", registry=arg.registry, addRegistry=False)
    return v
...
class Constant(ScalarBase):
    ...
    def __repr__(self):
        return f"Constant : {self.name} = {str(self.expr)}"
...
class Quantity(ScalarBase):
    ...
    def __repr__(self):
        return f"Quantity: {self.name} = {str(self.expr)} [{self.unit}] {self.type}"
...
class Variable(ScalarBase):
    ...
    def __repr__(self):
        return f"Variable: {self.name} = {str(self.expr)}"
...
class Expression(ScalarBase):
    ...
    def __repr__(self):
        return f"Expression: {self.name} = {str(self.expr)}"
...
class Position(VectorBase):
    ...
    def __repr__(self):
        return f"Position : {self.name} = [{str(self.x)} {str(self.y)} {str(self.z)}]"
...
class Rotation(VectorBase):
    ...
    def __repr__(self):
        return f"Rotation : {self.name} = [{str(self.x)} {str(self.y)} {str(self.z)}]"
...
class Scale(VectorBase):
    ...
    def __repr__(self):
        return f"Scale : {self.name} = [{str(self.x)} {str(self.y)} {str(self.z)}]"
...
class Matrix:
    ...
    def __init__(self, name, coldim, values, registry, addRegistry=True):
        ...
        for i, v in enumerate(values):
            self.values.append(
                Expression(
                    f"matrix_expr_{name}_idx{i}_val",
    ...
    def __repr__(self):
        return f"Matrix : {self.name} = {str(self.coldim)} {str(self.values)}"
