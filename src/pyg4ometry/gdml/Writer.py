...
class Writer:
    ...
    def writeGmadTester(
        self, filenameGmad, filenameGDML, writeDefaultLattice=False, preprocessGDML=True
    ):
...
"""
    ...
    def writeDefine(self, define):
        if isinstance(define, _Defines.Constant):
            ...
            raise Exception(f"Unrecognised define type: {type(define)}")
    ...
    def writeMaterialProps(self, material, oe):
        for pname, pref in material.properties.items():
            ...
            if not isinstance(pref, _Defines.Matrix):
                raise ValueError(
                    f"Only references to matrices can be used for material property {pname}"
                )
    ...
    def writeLogicalVolume(self, lv):
        ...
        for dv in lv.daughterVolumes:
            if dv.type == "placement":
                ...
                raise ValueError(f"Unknown daughter volume type: {dv.type}")
            we.appendChild(dve)
