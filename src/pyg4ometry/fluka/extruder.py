    ...
    def build_cgal_polygons(self):
        ...
        for region in self.regions:
            ...
            for point in self.regions[region]:
                self.cgalpolys[region].push_back(Point_2.Point_2_EPECK(point[0], point[1]))

        outer_polygon = self.cgalpolys[self.boundary]
        outer_polygon_hole = pyg4ometry.pycgal.Polygon_with_holes_2.List_Polygon_with_holes_2_EPECK()

        for region in self.regions:
            if self.boundary != region:
                pyg4ometry.pycgal.CGAL.difference(outer_polygon,
                                                  self.cgalpolys[region],
                                                  outer_polygon_hole)
                outer_polygon = outer_polygon_hole[0]
                self.decomposed[region] = pyg4ometry.pycgal.CGAL.PolygonDecomposition_2_wrapped(self.cgalpolys[region])
        self.decomposed[self.boundary] = pyg4ometry.pycgal.CGAL.PolygonWithHolesConvexDecomposition_2_wrapped(outer_polygon_hole[0])
