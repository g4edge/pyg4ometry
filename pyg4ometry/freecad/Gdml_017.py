...
def DocumentToGdml(materialMap = {"default":"G4_Cu"}) :
    ...
    for rootObject in rootObjects :
        ...
        if rootObject.TypeId == 'Part::Feature' : 
            meshes.append([rootLabel,BodyToMesh(rootObject)])
        ...
        if rootObject.TypeId == 'App::Part' : 
            meshes.append([rootLabel,PartToMesh(rootObject)])

    MeshListToGdml(doc.Label,meshes,materialMap)        
    
    return meshes

def DocumentStructure() :
    doc = _FreeCAD.activeDocument()
...
def ObjectRecursion(obj, nameDict, labelDict) : 
    if obj.TypeId == "App::Origin" : 
        return

    try : 
        nameDict[obj.Name] = nameDict[obj.Name] + 1
    except KeyError :
        nameDict[obj.Name] = 1

    try : 
        labelDict[obj.Label] = labelDict[obj.Label] + 1
    except KeyError :
        labelDict[obj.Label] = 1

#    print str(obj.TypeId)+" "+obj.Label+" "+obj.Name
