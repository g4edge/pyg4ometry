...
def simpleDetector(vtkViewer=True, gdmlWriter=True):
    ...
    pmt_position = _pyg.geant4.ParameterVector(
        "pmt_position", [zero, zero, cc_length / 2.0 + pmt_length / 2.0 + safety]
    )
    ...
    bas_position = _pyg.geant4.ParameterVector(
        "bas_position",
        [zero, zero, cc_length / 2.0 + pmt_length + bas_length / 2.0 + 2 * safety],
    )
    ...
    cc_solid = _pyg.geant4.solid.Box(
        "cc_solid", cc_xsize / 2.0, cc_ysize / 2.0, cc_length / 2.0
    )
    cc_logical = _pyg.geant4.LogicalVolume(cc_solid, cc_material, "cc_logical")
    cc_physical = _pyg.geant4.PhysicalVolume(
        [0, 0, 0], [0, 0, 0], cc_logical, "cc_physical", world_logical
    )

    pmt_solid = _pyg.geant4.solid.Tubs(
        "pmt_solid", 0, pmt_radius, pmt_length / 2.0, 0, _math.pi * 2
    )
    pmt_logical = _pyg.geant4.LogicalVolume(pmt_solid, pmt_material, "pmt_logical")
    pmt_physical = _pyg.geant4.PhysicalVolume(
        [0, 0, 0], pmt_position, pmt_logical, "pmt_physical", world_logical
    )

    bas_solid = _pyg.geant4.solid.Tubs(
        "bas_solid", 0, bas_radius, bas_length / 2.0, 0, _math.pi * 2
    )
    bas_logical = _pyg.geant4.LogicalVolume(bas_solid, bas_material, "bas_logical")
    bas_physical = _pyg.geant4.PhysicalVolume(
        [0, 0, 0], bas_position, bas_logical, "bas_physical", world_logical
    )
