...
def Test(
    ...
    tp = _g4.PhysicalVolume([_np.pi / 4, 0, 0], [0, 20, 0], pl, "p_pv1", wl, reg)
    ...
    w = _gd.Writer()
    w.addDetector(reg)
    w.write(outputPath / "T012_geant4GenericPolycone2Fluka.gdml")

    # fluka conversion
    if not bakeTransforms:
        outputFile = outputPath / "T012_geant4GenericPolycone2Fluka.inp"
    else:
        outputFile = outputPath / "T012_geant4GenericPolycone2Fluka_baked.inp"
    ...
    f.write(str(outputPath / "T012_geant4GenericPolycone2Fluka.flair"))
