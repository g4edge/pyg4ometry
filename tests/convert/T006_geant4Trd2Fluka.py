...
def Test(
    ...
    tx1 = _gd.Constant("tx1", "75", reg, True)
    tx2 = _gd.Constant("tx2", "25", reg, True)
    ty1 = _gd.Constant("ty1", "50", reg, True)
    ty2 = _gd.Constant("ty2", "75", reg, True)
    tz = _gd.Constant("tz", "75.0", reg, True)
    ...
    w = _gd.Writer()
    w.addDetector(reg)
    w.write(outputPath / "T006_geant4Trd2Fluka.gdml")

    # fluka conversion
    if not bakeTransforms:
        outputFile = outputPath / "T006_geant4Trd2Fluka.inp"
    else:
        outputFile = outputPath / "T006_geant4Trd2Fluka_baked.inp"
    ...
    f.write(str(outputPath / "T006_geant4Trd2Fluka.flair"))
