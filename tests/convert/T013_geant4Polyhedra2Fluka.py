...
def Test(vis=False, interactive=False, fluka=True):
    ...
    w.addDetector(reg)
    w.write(
        _os.path.join(_os.path.dirname(__file__), "T013_geant4Polyhedra2Fluka.gdml")
    )
    ...
    if fluka:
        ...
        w.addDetector(freg)
        w.write(
            _os.path.join(_os.path.dirname(__file__), "T013_geant4Polyhedra2Fluka.inp")
        )

    # flair output file
    f = _fluka.Flair("T013_geant4Polyhedra2Fluka.inp", extentBB)
    f.write(
        _os.path.join(_os.path.dirname(__file__), "T013_geant4Polyhedra2Fluka.flair")
    )
