...
def Test(vis=False, interactive=False, fluka=True, outputPath=None, refFilePath=None):
    ...
    if fluka:
        ...
        w = _fluka.Writer()
        w.addDetector(freg)
        w.write(outputPath / "T108_geant4ReplicaZ2Fluka.inp")

        # flair output file
        f = _fluka.Flair("T108_geant4ReplicaZ2Fluka.inp", extentBB)
        f.write(outputPath / "T108_geant4ReplicaZ2Fluka.flair")
    ...
    if refFilePath is not None:
        assert _fc.cmp(refFilePath, outputPath / "T108_geant4ReplicaZ2Fluka.inp", shallow=False)
