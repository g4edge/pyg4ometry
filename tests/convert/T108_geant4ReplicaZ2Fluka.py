...
def Test(vis=False, interactive=False, fluka=False):
    ...
    mbl = _g4.ReplicaVolume(
        "mbl", bl, ml, _g4.ReplicaVolume.Axis.kZAxis, 8, 100, 0, reg, True, "mm", "mm"
    )
    ...
    w = _gd.Writer()
    w.addDetector(reg)
    w.write(_os.path.join(_os.path.dirname(__file__), "T108_geant4ReplicaY2Fluka.gdml"))
    ...
    if fluka:
        ...
        w.addDetector(freg)
        w.write(
            _os.path.join(_os.path.dirname(__file__), "T108_geant4ReplicaZ2Fluka.inp")
        )

        # flair output file
        f = _fluka.Flair("T108_geant4ReplicaZ2Fluka.inp", extentBB)
        f.write(
            _os.path.join(_os.path.dirname(__file__), "T108_geant4ReplicaZ2Fluka.flair")
        )
