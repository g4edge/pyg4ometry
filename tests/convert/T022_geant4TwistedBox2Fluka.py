...
def Test(vis=False, interactive=False, fluka=True):
    ...
    w.addDetector(reg)
    w.write(
        _os.path.join(_os.path.dirname(__file__), "T022_geant4TwistedBox2Fluka.gdml")
    )
    ...
    if fluka:
        ...
        w.addDetector(freg)
        w.write(
            _os.path.join(_os.path.dirname(__file__), "T022_geant4TwistedBox2Fluka.inp")
        )

    # flair output file
    f = _fluka.Flair("T022_geant4TwistedBox2Fluka.inp", extentBB)
    f.write(
        _os.path.join(_os.path.dirname(__file__), "T022_geant4TwistedBox2Fluka.flair")
    )
